...
export class MembersService {
  ...
  async getMemberByMemberName(memberName: string): Promise<Member> {
    const result = await this.membersRepo.getMemberByNameWithRoles(memberName);
    if (!result) await this.throwMemberNotFoundError();
    return result;
  }

  async getMemberByMemberId(memberId: string): Promise<Member> {
    const result = await this.membersRepo.getMemberByIdWithRoles(memberId);
    if (!result) await this.throwMemberNotFoundError();
